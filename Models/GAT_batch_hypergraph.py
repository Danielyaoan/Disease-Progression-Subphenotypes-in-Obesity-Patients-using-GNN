...
class FocalLossMultilabel(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        pt = torch.exp(-BCE_loss)  # pt = sigmoid(logits) if y=1 else 1 - sigmoid(logits)
...
class GAT_batch(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_embedding_dim, output_dim, num_layers, dropout_prob, heads):
        super(GAT_batch, self).__init__()

        self.num_layers = num_layers

        self.sages = torch.nn.ModuleList()
        self.sages.append(GATv2Conv(input_dim, hidden_dim, heads=heads))
        for i in range(num_layers - 2):
            self.sages.append(GATv2Conv(hidden_dim*heads, hidden_dim, heads=heads))
        self.sages.append(GATv2Conv(hidden_dim*heads, output_embedding_dim, heads=heads))

        self.fc = nn.Linear(output_embedding_dim*heads, output_dim)
    ...
    def forward(self, x, edge_index):
        for i in range(self.num_layers - 1):
            x = F.relu(self.sages[i](x=x, edge_index=edge_index))
            x = F.dropout(x, p=self.dropout_prob, training=self.training)
        x = self.sages[self.num_layers - 1](x=x, edge_index=edge_index)
        y = self.fc(x)
        return x, y #F.log_softmax(y, dim=1)
    
    """ 
    #new forward function for batching training
    ...
    def fit(self,  training_data, val_data, epochs, optimizer, batch_size, device, batch_log_size, batch_model_save_number, output_root_dir , models_dir, model_name, model_id, gamma):
        ...
        criterion = FocalLossMultilabel(gamma=gamma)
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                preds = (torch.sigmoid(out[:batch_size]) > 0.5).int()
                # out[:batch_size].argmax(dim=1)
                ...
                index+=1
                #save the model in every 1000 batches
                ...
                if index%batch_log_size == 1:
                    ...
                    with torch.no_grad():
                        ...
                        for batch in val_loader:
                            ...
                            val_total_samples += y_batch[:batch_size].numel()
                    ...
                    # calculate score
                    f1 = f1_score(true_test, pred_test, average='macro', zero_division=1)
                    ...
                    cm = multilabel_confusion_matrix(true_test, pred_test)
                    ...
                    if val_loss < best_val_loss:
                        best_val_loss = val_loss
                        best_model_val_loss = self.state_dict().copy()
                        no_improvement_val_loss = 0  # reset counter
                    else:
                        no_improvement_val_loss += 1  # increment counter

                    # Optional: also track early stopping on F1 score
                    if f1 > best_f1_score:
                        best_f1_score = f1
                        best_model_f1 = self.state_dict().copy()
                        no_improvement_f1 = 0
                    ...
                    if no_improvement_val_loss >= early_stop_patience:
                        print(f"\n⏹️ Early stopping triggered at epoch {epoch}, batch {index} — no improvement in validation loss for {early_stop_patience} steps.")
                        break
                    ####################################################################
                    ...
                    for i in range(num_classes):
                        ...
                        #     auroc = 0.0
                        try:
                            auroc = roc_auc_score(true_test, pred_test, average='macro')
                        except ValueError:
                            auroc = 0.0
