            ...
            for metric in param_grid["metric"]:
                # for max_iter in param_grid["max_iter"]:
                    # for tol in param_grid["tol"]:
                        ...
                        flat_score = compute_davies_bouldin_score_sklearn(X_padded, labels)

                        print(f"Flat Davies-Bouldin Score (Time-Series): {flat_score:.4f}")
                        ...
                        row.append([k, metric, n_init, db_score, flat_score])


    temp = pd.DataFrame(row, columns=['k', 'metric', 'n_init', 'db_score', 'flat_score'])
    temp.to_csv(f'./score_results/search_graphsage_k={k}_ninit_dbi.csv', index=False)
